...
static int alloc_mem_load(struct smart_amp_data *sad, uint32_t size)
{
	...
	if (!sad->model.data) {
		comp_cl_err(&comp_smart_amp, "alloc_mem_load(): sad->model.data rballoc failed");
		return -ENOMEM;
	}
...
static int smart_amp_copy(struct comp_dev *dev)
{
	...
	if (sad->feedback_buf->source->state == dev->state) {
		...
		smart_amp_process(dev, avail_frames, sad->feedback_buf,
				  sad->sink_buf, sad->config.feedback_ch_map);
