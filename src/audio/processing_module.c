...
static int proc_module_copy(struct comp_dev *dev)
{
	...
	if (cd->preload && dev->params.direction == SOF_IPC_STREAM_PLAYBACK) {
		/* if source buffer is fulfilled */
		if (source->avail >= process->min_source_bytes *
	...
	if (input_bytes < process->min_source_bytes) {
	...
	if (output_bytes < process->min_sink_bytes) {
	...
	copy_bytes = process->min_source_bytes * MIN(input_bytes, output_bytes);
...
static int proc_module_prepare(struct comp_dev *dev)
{
	...
	in_sample_bytes = process->min_source_bytes;
	out_sample_bytes = process->min_sink_bytes;

#if PROC_MODULE_DEBUG_TRACES
	trace_proc_module("proc_module_prepare(): process->min_source_bytes: %u",
			  process->min_source_bytes);
	trace_proc_module("proc_module_prepare(): process->min_sink_bytes: %u",
			  process->min_sink_bytes);
	...
	if (in_sample_bytes * config->periods_source > source->size) {
	...
	if (out_sample_bytes * config->periods_sink > sink->size) {
