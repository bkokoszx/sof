...
static void comp_free_data_blob(struct comp_dev *dev,
				struct comp_data_blob_handler *blob_handler)
{
	if (!blob_handler || !blob_handler->data)
		return;
...
void *comp_get_data_blob(struct comp_dev *dev,
			 struct comp_data_blob_handler *blob_handler,
			 size_t *size)
{
	comp_dbg(dev, "comp_get_data_blob()");

	if (size)
		*size = 0;

	if (!blob_handler) {
		comp_err(dev, "comp_get_data_blob(): !blob_handler");
		return NULL;
	}

	/* Function returns new data blob if available */
	if (comp_is_new_data_blob_available(dev, blob_handler)) {
		comp_dbg(dev, "comp_get_data_blob(): new data available");
	...
	/* If blob_handler->data is equal to NULL and there is no new data blob
	 * it means that component hasn't got any config yet. Function returns
	 * NULL in that case.
	 */
	if (!blob_handler->data) {
		comp_warn(dev, "comp_get_data_blob(): blob_handler->data is not set.");
		return NULL;
	}
...
bool comp_is_new_data_blob_available(struct comp_dev *dev,
				     struct comp_data_blob_handler
					*blob_handler)
{
	comp_dbg(dev, "comp_is_new_data_blob_available()");

	if (!blob_handler) {
		comp_err(dev, "comp_is_new_data_blob_available(): !blob_handler");
		return false;
	}
...
int comp_init_data_blob(struct comp_dev *dev,
			struct comp_data_blob_handler *blob_handler,
			uint32_t size, void *init_data)
{
	int ret;

	comp_free_data_blob(dev, blob_handler);

	if (!blob_handler) {
		comp_err(dev, "comp_init_data_blob(): !blob_handler");
		return -ENOMEM;
	}
...
int comp_data_blob_set_cmd(struct comp_dev *dev,
			   struct comp_data_blob_handler *blob_handler,
			   struct sof_ipc_ctrl_data *cdata)
{
	int ret = 0;

	comp_dbg(dev, "comp_data_blob_set_cmd() msg_index = %d, num_elems = %d, remaining = %d ",
	...
	if (!cdata->msg_index) {
		...
		if (!blob_handler->data_new) {
			comp_err(dev, "comp_data_blob_set_cmd(): blob_handler->data_new allocation failed.");
			return -ENOMEM;
		}
	...
	if (!cdata->elems_remaining) {
		comp_dbg(dev, "comp_data_blob_set_cmd(): final package received");
...
int comp_data_blob_get_cmd(struct comp_dev *dev,
			   struct comp_data_blob_handler *blob_handler,
			   struct sof_ipc_ctrl_data *cdata, int size)
{
	int ret = 0;

	comp_dbg(dev, "comp_data_blob_get_cmd() msg_index = %d, num_elems = %d, remaining = %d ",
		 cdata->msg_index, cdata->num_elems,
		 cdata->elems_remaining);
	...
	if (blob_handler->data) {
		...
		if (cdata->num_elems > size) {
			comp_err(dev, "comp_data_blob_get_cmd(): invalid cdata->num_elems %d",
				 cdata->num_elems);
			return -EINVAL;
		}
...
void comp_data_blob_handler_free(struct comp_dev *dev,
				 struct comp_data_blob_handler *blob_handler)
{
	comp_dbg(dev, "comp_data_blob_handler_free()");
