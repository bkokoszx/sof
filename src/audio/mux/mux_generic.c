			       ...
			       struct mux_look_up *lookup)
{
	...
	for (elem = 0; elem < lookup[0].num_elems; elem++) {
		source = sources[lookup[0].copy_elem[elem].stream_id];
		audio_stream_wrap(sink, lookup[0].copy_elem[0].dest);
		audio_stream_wrap(source, lookup[0].copy_elem[0].src);
	}
}
					     ...
					     uint32_t frames)
{
	...
	for (elem = 0; elem < lookup[0].num_elems; elem++) {
		...
		size_to_end = (char *)source->end_addr -
			(char *)lookup[0].copy_elem[elem].src;
		...
		size_to_end = (char *)sink->end_addr -
			(char *)lookup[0].copy_elem[elem].dest;
		tmp_frames = size_to_end / frame_bytes;
		/* check whether there is enough memory for tmp_frames and
		 * one more channel
		 */
					  ...
					  struct mux_look_up *lookup)
{
	...
	for (elem = 0; elem < lookup[0].num_elems; elem++) {
		...
		lookup[0].copy_elem[elem].src = (int32_t *)source->r_ptr +
			lookup[0].copy_elem[elem].in_ch;
		lookup[0].copy_elem[elem].src_inc = source->channels;

		lookup[0].copy_elem[elem].dest = (int32_t *)sink->w_ptr +
			lookup[0].copy_elem[elem].out_ch;
		lookup[0].copy_elem[elem].dest_inc = sink->channels;
	}
}
					  ...
					  struct mux_look_up *lookup)
{
	...
	for (elem = 0; elem < lookup[0].num_elems; elem++) {
		...
		lookup[0].copy_elem[elem].src = (int16_t *)source->r_ptr +
			lookup[0].copy_elem[elem].in_ch;
		lookup[0].copy_elem[elem].src_inc = source->channels;

		lookup[0].copy_elem[elem].dest = (int16_t *)sink->w_ptr +
			lookup[0].copy_elem[elem].out_ch;
		lookup[0].copy_elem[elem].dest_inc = sink->channels;
	}
}
			...
			struct mux_look_up *lookup)
{
	...
	while (frames) {
		...
		for (i = 0; i < frames_without_wrap; i++) {
			for (elem = 0; elem < lookup[0].num_elems; elem++) {
				src = (int16_t *)lookup[0].copy_elem[elem].src;
				dst = (int16_t *)lookup[0].copy_elem[elem].dest;
				*dst = *src;
				lookup[0].copy_elem[elem].src = src +
					lookup[0].copy_elem[elem].src_inc;
				lookup[0].copy_elem[elem].dest = dst +
					lookup[0].copy_elem[elem].dest_inc;
			}
		}
			...
			struct mux_look_up *lookup)
{
	...
	while (frames) {
		...
		for (i = 0; i < frames_without_wrap; i++) {
			for (elem = 0; elem < lookup[0].num_elems; elem++) {
				src = (int32_t *)lookup[0].copy_elem[elem].src;
				dst = (int32_t *)lookup[0].copy_elem[elem].dest;
				*dst = *src;
				lookup[0].copy_elem[elem].src = src +
					lookup[0].copy_elem[elem].src_inc;
				lookup[0].copy_elem[elem].dest = dst +
