					 ...
					 struct sof_ipc_comp *comp)
{
	...
	ret = comp_init_data_blob(dev, cd->model_handler,
				  INITIAL_MODEL_DATA_SIZE, NULL);
	if (ret < 0) {
					  ...
					  int size)
{
	...
	case SOF_DETECT_TEST_CONFIG:
		...
		break;
	case SOF_DETECT_TEST_DATA_BLOB:
		ret = comp_data_blob_get_cmd(dev, cd->model_handler, cdata, size);
		break;
	default:
...
static int test_keyword_prepare(struct comp_dev *dev)
{
	...
	cd->data_blob = comp_get_data_blob(dev, cd->model_handler,
					   &cd->data_blob_size);
	if (cd->data_blob)
		cd->data_blob_crc = crc32(0, cd->data_blob, cd->data_blob_size);
